#!/usr/bin/env python3
"""
agent.py - AgentRIC (Ollama + MCP stdio)

Fixes:
- Pydantic JSON schema error (CallableSchema) by excluding ToolRuntime from schema using InjectedToolArg

Features:
- Local/remote Ollama via ChatOllama
- MCP tools loaded via langchain_mcp_adapters using MultiServerMCPClient (stdio)
- Robust tool result normalization (ToolMessage / BaseMessage / dict / str)
- Middleware guardrails (allowlist / dry-run / rate limit / audit)

Requirements (rough):
  pip install python-dotenv anyio langchain-ollama langchain langchain-mcp-adapters mcp
"""
...
# IMPORTANT: InjectedToolArg prevents schema generation for runtime (fixes CallableSchema error)
...
# ============================================================
# MCP Bridge using MultiServerMCPClient (stdio)
# ============================================================
...
class _MCPBridge:
    ...
    Uses a background AnyIO portal to avoid nested asyncio issues.
    """

    def __init__(self) -> None:
        self._lock = threading.Lock()

        # BEFORE: self._portal: Optional[BlockingPortal]
        # NOW:
    ...
    def _ensure_runner(self) -> None:
        """Start a background AnyIO BlockingPortal (works across AnyIO versions)."""
        with self._lock:
            if self._runner_started and self._portal is not None:
                return

            # AnyIO v4 returns a context manager
    ...
    async def _async_init_client(self) -> None:
        ...
        self._client = MultiServerMCPClient(server_cfg)

        # Some versions require __aenter__ to start subprocesses
    ...
    async def _async_load_tools(self) -> list:
        ...
        # Different adapter versions accept different args
        try:
            tools = await load_mcp_tools(self._client, server_name=MCP_SERVER_NAME)
        except TypeError:
            tools = await load_mcp_tools(self._client)
...
def mcp_call_tool(
    ...
    """Call an MCP tool by name. Accepts args as dict OR args_json as JSON string."""
    call_args: Dict[str, Any] = {}

    if isinstance(args, dict):
        call_args = args
    elif args_json and isinstance(args_json, str):
        try:
