...
#    (messages live in state automatically; we add extra fields)
# =========================
...
def search_products(query: str, runtime: ToolRuntime[Context]) -> List[Dict[str, str]]:
    ...
    _ = runtime.context.user_id  # demonstrate runtime context access
    q = query.lower()
...
# =========================
# 4) Structured output schema for ToolStrategy
# =========================
...
def parse_json_from_last_message(resp: dict) -> dict | None:
    """LangChain 1.2.9 often returns only messages; parse JSON from last assistant message."""
...
# =========================
# 6) Middleware: dynamic system prompt + tool gating + tool error handling + memory sync
# =========================
@wrap_model_call
def dynamic_system_prompt(request: ModelRequest, handler) -> ModelResponse:
    """Build system prompt dynamically from runtime context + memory."""
...
Memory you may use:
- last_products: cached candidates from search_products
- stock_map: cached stock results from check_stock

Policy:
- If last_products is missing OR user asks for "latest", call search_products("wireless headphones").
- Otherwise reuse last_products.
- If stock_map missing/incomplete, call check_stock for the top candidates.
- Use observations before answering.
- Return ONLY valid JSON matching:
  {{"top_products":[{{"name":"...","retailer":"..."}}...],
    "stock_status":{{"Product Name":{{"in_stock":true/false,"note":"..."}}}},
    "summary":"..."}}.
...
def gate_tools_by_state(request: ModelRequest, handler) -> ModelResponse:
    """
    Dynamically enable/disable tools based on memory.
    - If we don't have products yet (or user asked latest), only allow search_products.
...
def handle_tool_errors(request: ToolCallRequest, handler: Callable[[ToolCallRequest], ToolMessage]) -> ToolMessage:
    """Return tool errors back to the model as ToolMessage so it can recover."""
    try:
        return handler(request)
...
def sync_memory_from_tool_observations(request: ModelRequest, handler) -> ModelResponse:
    """
    Update custom memory fields from ToolMessage observations already in state.
    Runs at the start of each model call (robust for LangChain 1.x).
    """
    st = _state_as_custom(request.state)
    ...
    # Scan recent tool messages; our tools return python objects -> stored as strings
    for m in msgs[-30:]:
        if getattr(m, "type", None) == "tool":
            content = getattr(m, "content", "")
            # search_products returns list[dict]
            if isinstance(content, str) and content.strip().startswith("[") and "retailer" in content:
                try:
                    parsed = json.loads(content.replace("'", '"'))  # best-effort
                    if isinstance(parsed, list):
                        last_products = parsed
                except Exception:
                    pass
            # check_stock returns dict that includes product_name
            if isinstance(content, str) and content.strip().startswith("{") and "product_name" in content:
                try:
                    parsed = json.loads(content.replace("'", '"'))  # best-effort
                    if isinstance(parsed, dict) and "product_name" in parsed:
                        pname = parsed["product_name"]
                        stock_map[pname] = parsed
                except Exception:
                    pass
...
# =========================
# 7) Model + agent (ToolStrategy + memory)
# =========================
model = ChatOllama(
    model=MODEL,
    base_url=OLLAMA_URL,
...
agent = create_agent(
    model=model,
    tools=ALL_TOOLS,  # must register all tools up front
    middleware=[
        sync_memory_from_tool_observations,  # memory
        dynamic_system_prompt,               # dynamic prompt from memory + runtime ctx
        gate_tools_by_state,                 # enable/disable tools based on memory
        handle_tool_errors,                  # tool error handling
    ],
    checkpointer=checkpointer,              # message + state persistence (thread_id)
    context_schema=Context,                 # runtime context for tools/middleware
    response_format=ToolStrategy(ResponseFormat),  # artificial tool calling for structured output
)


# =========================
# 8) Invocation (state update: messages)
# =========================
...
print("\n=== ASSISTANT (turn 1) ===")
print(parse_json_from_last_message(resp1) or resp1["messages"][-1].content)

resp2 = agent.invoke(
    {"messages": [{"role": "user", "content": "Only list the in-stock ones."}]},
    config=config,
    context=ctx,
)

print("\n=== ASSISTANT (turn 2) ===")
print(parse_json_from_last_message(resp2) or resp2["messages"][-1].content)
