...
AUTO_START = os.getenv("FLEXRIC_AUTO_START", "0").strip() == "1"
SILENCE_STDOUT = os.getenv("FLEXRIC_SILENCE_STDOUT", "1").strip() == "1"
...
def _silence_native_stdout():
    """
    For stdio MCP, stdout must contain only JSON-RPC.
    Some native FlexRIC/xApp code prints to stdout, which corrupts MCP frames.
    Redirect fd=1 to stderr temporarily around native SDK calls.
    """
...
if __name__ == "__main__":
    # For stdio MCP, avoid noisy native startup on stdout by default.
    if AUTO_START:
        try:
            _ensure_started()
